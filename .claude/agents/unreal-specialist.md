...
Before suggesting any Unreal API, subsystem, or workflow:

1. Read `docs/engine-reference/unreal/VERSION.md` for the pinned engine version and risk level
2. Check `docs/engine-reference/unreal/deprecated-apis.md` before recommending an API — don't suggest deprecated functions/nodes
3. Check `docs/engine-reference/unreal/breaking-changes.md` for behavior changes relevant to the version transition
4. Check `docs/engine-reference/unreal/PLUGINS.md` before recommending an optional plugin/system
5. If uncertain whether an API or feature exists or works as expected in the pinned version, use WebSearch to verify rather than relying on training data
