...
Feature: **Technical Setup gate PASSED** (2026-07-20). Pre-Production phase active. Report: `production/gate-checks/2026-07-20-preprod-final-pass.md`.
Task: **Signature Combat Chain Spike — CLOSED (2026-07-21, commit 8753883)**. Engine feasibility proven. Overdrive automation tests 2/2 PASS. HUD Lunar Fracture v2 verified at 16:9 and 21:9. Spike not promoted to production — remains in `prototypes/`. Next: (1) Playtest scenario **locked 2026-07-23** — solo self-test (Provisional, not final gate) at `production/qa/playtest/2026-07-23-signature-chain-selftest.md`; not yet executed (needs 3-day cooldown from last logic touch, earliest 2026-07-24). External cold-player 3–5 person test still queued separately, required before promotion. (2) **GDD re-reviews DONE 2026-07-23**: combo-tension-gauge.md, luna-overdrive.md, spell-casting-base.md all → Approved. MVP GDD count now genuinely 9/9 Approved. (3) **ADR suite expansion DONE 2026-07-23**: ADR-0005 (Camera/SpringArm), ADR-0006 (Enemy AI/BT), ADR-0007 (Dash/Just-Dodge) all written, Proposed, registry updated. Key findings baked into these ADRs — must inform next implementation session:
   - **Real bug found (Camera)**: `MoonCharacterBase.cpp:43-44` rotation flags (`bUseControllerRotationYaw=false`, `bOrientRotationToMovement=true`) contradict Approved GDD Rule 6/player-movement.md Rule 2 (instant snap-to-camera-yaw facing for strafe-aim). Confirmed via `git blame` as unmodified scaffolding boilerplate (commit `9724f44`), never intentionally set. GDD is correct, code is the bug. Tracked as spawned background task "Fix character rotation flags to match GDD Rule 2" (`task_0964acf9`) — NOT yet fixed.
   - **Real implementation gap (Dash)**: `UMoonGameplayAbility_Dash::CheckJustDodge()` is a complete no-op stub (`// TODO`). The entire Just-Dodge → `State.Executable` → F-key-execution core loop does not exist in code yet — distinct from and not proven by the Signature Combat Chain spike (that spike never touched Dash/Just-Dodge). ADR-0007 designs the fix (sphere-overlap query architecture, extends ADR-0006 with `IsTelegraphingAttack()`/`GetAttackCommittedTime()`/`MeleeAttackRange`) but no code written yet.
...
- Added three static 44×44 spell-slot frames above the mana bar, with Blackhole, Fire, and Lightning 32×32 icons at Z=3. They are presentation-only until the cooldown runtime data contract is implemented.
- Verified every overlay brush points to the canonical `/Game/Moon/UI/Art` texture assets, their slots use relative bottom anchors, compilation succeeded, and `WBP_CombatHUD` saved successfully. Remaining: human PIE visual check at target HUD scales; do not alter the existing graph with `write_graph_dsl`.
- **Reference-layout revision**: user rejected the first scale. Repositioned to the supplied dark action-RPG composition: Health is now a left-bottom vertical fill (`BottomToTop`) inside its 250×250 crystal frame; Mana is a 280×12 fill inside a 380×95 thin frame; spell slots moved below the mana frame and enlarged to 58×58 with 48×48 icons. Static Health/Mana value text is `Collapsed`; all gameplay bindings remain untouched. Recompiled and saved successfully.

## Session Extract — HUD art placement attempt (Codex, 2026-07-20)
- Opened `/Game/Moon/UI/WBP_CombatHUD` and verified the existing health, mana, tension, dash, and low-health vignette widgets remain intact.
- The imported texture assets are available under `/Game/Moon/UI/Art`, but the current Windows automation exposed the level editor and widget designer as overlapping targets. A trial drag was immediately undone; no HUD or level change was saved, and the low-health vignette deletion prompt was cancelled.
- Resume with direct Unreal MCP or manual UMG Designer access before attempting the actual Image widgets: add non-interactive Image overlays for `F_UI_HUD_HealthCrystalFrame` and `F_UI_HUD_ManaFrame_Horizontal`, then add spell-frame/icon overlays. Preserve existing ProgressBar bindings and the `Border_LowHealthVignette` event binding.

## Session Extract — HUD texture import retry (Codex, 2026-07-20)
- Successfully imported and saved 12 individual HUD textures into UE5.8 at `/Game/Moon/UI/Art`: health/mana, spell-slot, dash/tension frames, Blackhole/Fire/Lightning, and five state/prompt icons. Local commit: `7a825cc`.
- The texture sources remain in `Moon/Content/Moon/UI/Art/ToImport/`; UE automatically discovered duplicate source-side `.uasset` files under generated/source folders as well. They were deliberately left untouched because cleanup would delete editor assets and needs explicit user authorization.

## Session Extract — Compact HUD visual source package (Codex, 2026-07-20)
- User approved creation of a separable UMG art package. Added 13 alpha PNG source assets under `Moon/Content/Moon/UI/Art/Generated/` and a clean import subset under `Moon/Content/Moon/UI/Art/ToImport/` (health/mana frames, spell-slot/dash/tension frames, three spell icons, and contextual state icons).
- Added `Moon/Content/Moon/UI/Art/README.md` with UE texture import settings (UI / NoMipmaps / TC Editor Icon), widget mapping, and event-driven animation contract.
- Added `design/ux/ue58-combat-hud-implementation.md` with the UMG hierarchy, compact PC layout rule, and verification checklist.
- Unreal Editor was opened and the Content Browser Import dialog reached, but the Windows automation target became unavailable while its native file modal was active. PNG assets are ready under the project Content path; import the `ToImport/` files manually or resume via a working editor MCP session.
...
## Session Extract — /code-review + /story-done 2026-08-12
- Ran /code-review on Moon/Source/Moon/Character/MoonCharacterBase.h/.cpp (Stories 001-005 combined). Engine-specialist pass confirmed bPauseAnims, TRACE_CPUPROFILER_EVENT_SCOPE bare-token syntax, and LaunchCharacter bXYOverride/bZOverride semantics all correct against installed UE5.8 headers (previously flagged "unverified" in code comments — now confirmed).
- Verdict CHANGES REQUIRED (3 items) — user chose fix-now: split Tick() (~150 lines, 7 responsibilities) into UpdateJumpTimers/UpdateResourceRegen/UpdateJumpFeelGravity/UpdateJumpAnimState; removed a function-local `static float DebugLogAccum` bug (shared across all instances of the class, not per-instance); stripped leftover `[MoonDebug]` UE_LOG noise from Move()/input paths (kept only real failure-path warnings). Updated 2 unit test regex anchors (airborne_and_grace_windows_test.ps1, tuning_clamp_and_joint_bound_test.ps1) that were coupled to the old Tick()-body location of the extracted logic.
- UBT build Succeeded (5/5). All 9 movement test scripts re-verified PASS after the refactor. Committed + pushed: commit `1561ecf`.
- Ran /story-done on Story 002 — verdict COMPLETE WITH NOTES (4/4 AC passing, 2 ADVISORY deviations: TR-mov-005's MaxWalkSpeed-override half intentionally out of scope per story's own scope note; today's Tick() refactor spans code from Stories 002-005). Story file updated: Status: Complete, Last Updated: 2026-08-12, Completion Notes added.
...
## Session Extract — repo sync + verification 2026-08-12 (follow-up)
- Verified uncommitted `MoonGameplayAbility_Dash.cpp` change: legacy `SetMovementMode()` -> `SetMovementModeWithCustomMode()`, matches ADR-0007 Migration Plan item 6 and `deprecated-apis.md:183` exactly. No UBT/engine available locally to compile; `movement_lock_contract_test.ps1`, `movement_foundation_contract.ps1`, `camera_yaw_facing_test.ps1` all live-re-run PASS. Committed + pushed (`f54a85f`) — only the 5 relevant tracked files, none of the ~700 untracked files.
- Fixed stale "Story 001: In Progress" dependency text in Story 003/004 (Story 001 had already flipped to Complete-with-notes this session; text wasn't updated at the time).
- Closed Story 001 AC-4 (non-root-motion locomotion) without editor/PIE: `BP_MoonCharacter.uasset` plays 6 raw ParagonAurora AnimSequences via `AnimationSingleNode` (no AnimBP). Each `.uasset`'s asset-registry tag block has a plaintext `bEnableRootMotion`/`False` pair readable by direct binary offset scan — confirmed for all 6 (Idle, Jog_Fwd, Jump_Apex, Jump_Land, Jump_Recovery, Jump_Start). Story 001 now 4/4 AC passing.
- `production/stage.txt` (Pre-Production) and latest gate-check (`2026-08-12-preprod-to-production.md`, FAIL) confirmed consistent — no drift.
- Still open: Story 004 AC-1/AC-3/AC-4 (Visual/Feel, genuinely need human PIE playtest — no static substitute exists for "feel"). N>=100 benchmark (Story 005 AC-4) still needs reference-machine PIE. 713 untracked files (installers/assets/QA outputs) still need scoping before any bulk add. Production gate stays FAIL/Pre-Production until those close.

## Session Extract — 713 untracked file cleanup 2026-08-12
- Classified all 713 untracked files into 3 buckets: (A) 504 files / 987MB — `Moon/Content/Free_Magic/` + `Free_Spells/` marketplace packs incl. their `__ExternalActors__`/`__ExternalObjects__` World Partition stubs, same class as the already-ignored ParagonAurora/Ghoul/GoodSky packs but not yet covered by `.gitignore`; (B) 1 file — `docs/registry/architecture.yaml.tmp.13448.bc1ec0e0fd01`, a dead process's PID-named temp file; (C) 223 files — real project output (multi-tool skill/agent mirrors for Antigravity `.agents/skills/`+Codex `.codex/agents/`+new Claude `.claude/skills/unreal-*`, combat HUD art under `Moon/Content/Moon/UI/Art/`, QA/gate docs, `skills-lock.json`, `design/ux/combat-hud.md`).
- User confirmed "전체 진행" (recommended option). Executed: added Free_Magic/Free_Spells + their ExternalActors/Objects to `.gitignore`, deleted the tmp file (commit `4540fbb`); committed all 223 bucket-C files (commit `04e58ae`, LFS uploaded 15 objects). Pushed both. `git status` now clean (0 untracked/modified).
- Note for future packs: if a new marketplace pack gets placed into a level (World Partition actor placement), its `__ExternalActors__`/`__ExternalObjects__` subfolder needs its own explicit `.gitignore` line — the top-level `Moon/Content/<Pack>/` pattern does not cover it. None of the currently-ignored packs (Paragon*, Ghoul, GoodSky) have been placed in a level yet, so this hasn't recurred beyond Free_Magic/Free_Spells.
- Next recommended: repo state fully synced and clean. Resume Player Movement Foundation Fixes PIE debt (Story 004 AC-1/3/4, Story 005 AC-4 N>=100 bench) when reference-machine/editor access is available, or move to Camera System / Dash-Evasion epics per `production/epics/index.md`. Production gate stays FAIL/Pre-Production until PIE debt closes.

## Session Extract — Story 004 PIE closure 2026-08-12
- UE 5.8 editor confirmed installed on this machine (`C:\Program Files\Epic Games\UE_5.8\Engine\Binaries\Win64\UnrealEditor.exe`) and launched this session. `unreal-mcp` config exists in `.mcp.json` (points at `127.0.0.1:8000`) and the port did come up once the editor finished loading (HTTP 405, i.e. server present) — but no `unreal-mcp` tool was actually wired into this Claude Code session's toolset (`ToolSearch` found nothing), so PIE could not be driven or screenshotted from here directly.
- User played the build directly in the now-running editor and reported AC-1/AC-3/AC-4 all PASS against the reviewer checklist in `production/qa/evidence/presentation-only-hitstop-rewrite-evidence.md`. Story 004 updated to 4/4 AC passing; evidence doc got a new "Human PIE playtest result (2026-08-12)" section recording the pass/fail-level result (no clip captured, no further per-scenario detail beyond the checklist).
- Still open: Story 005 AC-4 N>=100 benchmark still needs a reference-machine PIE run (different requirement — needs actual perf numbers, not just pass/fail). If the editor stays up and someone wires an actual `unreal-mcp` connection into a Claude Code session, PIE-driven checks become directly automatable from here instead of relying on manual human report.
- Next recommended: Player Movement Foundation Fixes epic is now fully closed (all 5 stories Complete, all non-advisory AC passing). Story 005 AC-4 benchmark remains the only open PIE-dependent item project-wide. Production gate still FAIL/Pre-Production (see `2026-08-12-preprod-to-production.md`) — re-run `/gate-check` if attempting Production again, or move to Camera System / Dash-Evasion epics per `production/epics/index.md`.

## Session Extract — SetMovementModeWithCustomMode correction 2026-08-12 (IMPORTANT — retracts prior "verified" claim)
- **Correction to this session's own earlier work**: commit `f54a85f` (and its accompanying "verified against ADR-0007/deprecated-apis.md" claim in this file's earlier entry) was WRONG. `SetMovementModeWithCustomMode()` does not exist on this project's installed UE 5.8 engine — a real UBT compile failed with `error C2039`. The actual header (`CharacterMovementComponent.h:1276`) only has `SetMovementMode(EMovementMode, uint8 NewCustomMode = 0)`, not deprecated. Root cause: `docs/engine-reference/unreal/deprecated-apis.md:183` asserted the false replacement-function claim, and the prior session cross-checked the fix against that doc instead of the real installed header — doc-vs-doc agreement was mistaken for verification. **Lesson logged: for this project's post-cutoff engine (5.8), "matches the doc" is not sufficient verification for an API call — only a real compile (or a genuine read of the installed engine header, as done here) counts.**
- Fixed in commit `cb7a626`: `MoonGameplayAbility_Dash.cpp` reverted to `SetMovementMode()`; `deprecated-apis.md`'s false row removed with a correction note; `docs/architecture/0007-dash-evasion-just-dodge.md`'s Engine Compatibility table and Migration Plan item 6 corrected (item 6 struck through, marked RETRACTED).
- Not yet corrected (lower-stakes, hedged references, not scope of this pass): `design/gdd/player-movement.md`, `docs/architecture/0009-player-movement-runtime-contract.md`, `docs/architecture/architecture-review-2026-07-27.md`, `production/epics/player-movement-foundation-fixes/story-003-...md`, `production/session-logs/session-log.md` all mention `SetMovementModeWithCustomMode()` in passing (mostly "we deliberately avoid this API" framing, already hedged as unverified) — a full sweep of these was out of scope for this pass; flag for cleanup if anyone reads those sections and gets confused.
- This was caught only because building the Story 005 AC-4 benchmark helper (`Moon_BenchmarkSpawnMovers`, added same session) forced an actual UBT compile for the first time this project cycle — Live Coding + never having run a clean UBT build on this file before is why it went uncaught since `f54a85f`.

## Session Extract — Story 005 AC-4 closed via real unreal-mcp 2026-08-14
- Found `unreal-mcp` genuinely live at `127.0.0.1:8000/mcp` (the running `UnrealEditor.exe`) — no Claude Code tool wraps it, but it's a real MCP server reachable via raw HTTP JSON-RPC. Used `Bash`/`curl` directly against the protocol (`initialize` → `tools/list` → `list_toolsets`/`describe_toolset`/`call_tool`) since no wired tool existed.
- Exposed toolsets: `EditorToolset.EditorAppToolset` (PIE start/stop, cvars-search-only, viewport/actor selection, screenshots), `EditorToolset.LogsToolset` (read log), `editor_toolset.toolsets.scene.SceneTools` (place/remove actors, folders, level load), `editor_toolset.toolsets.programmatic.ProgrammaticToolset` (sandboxed Python `execute_tool_script` that loops/glues other tool calls — modules limited to `json/math/datetime/re/time/copy`), `AutomationTestToolset`, GAS/Niagara/PCG/Sequencer/etc toolsets. **No stat-overlay, cvar-SET, console-command-exec, or Insights/profiling tool exists anywhere in this bridge** — confirmed by reading every toolset's tool list, not by assumption.
- Used `ProgrammaticToolset.execute_tool_script` to loop `SceneTools.add_to_scene_from_asset` 100× spawning `/Game/Moon/BP_MoonCharacter` in a grid (confirmed `{"spawned":100}`), then `EditorAppToolset.StartPIE` on `L_CombatTest`. `LogsToolset.GetLogEntries` confirmed clean start (100 clothing-actor recreations, matching count) and zero errors/warnings over an 80+s sustained run (2000+ frames, past the AC's 300-frame ask).
- Since the bridge can't read `stat unit`, asked the user to type `stat unit` themselves in the already-running 100-actor PIE session and screenshot it. Got real numbers: **Frame 26.50ms, Game 26.98ms**, Draw 6.97ms, RHIT 10.20ms, GPU 16.05ms, Input 25.39ms, Mem 3.84GB, VRAM 2.09/7.05GB, RenderRes 50.0% (461×262), Draws 4430, Prims 3608.1K. Flagged the 50% render-res scaling as making Draw/RHIT/GPU non-representative of target hardware — Frame/Game (CPU, unaffected by render-res) are the numbers actually relevant to AC-4's movement-tick scope.
