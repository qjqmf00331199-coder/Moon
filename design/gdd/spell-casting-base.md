...
> **Status**: Approved (2026-07-17 full design-review — 5 specialists, blocking items resolved in-session)
> **Author**: user + Claude Code (full review mode)
...
7. **마나 회복은 패시브 + 코어 적출 보너스**: 시간경과 패시브 회복(Tuning Knob `ManaRegenRate`)이 기본 소스. 추가로 Health/Damage Core의 `OnExecuted` 이벤트(코어 적출 성공)를 이 문서가 구독해 Mana를 즉시 MaxMana로 100% 회복시킨다 — Health/Damage Core Rule 9가 "회복량 정의는 이 문서 소유"라고 명시한 지점을 여기서 확정.
...
10. **오버드라이브/코어적출 등 외부 시스템을 위한 코스트·쿨다운 우회 훅 노출**: `bManaCostBypassed`/`bCooldownBypassed` 오버라이드 플래그를 이 시스템이 노출한다 — 언제, 얼마나 오래 이 플래그가 켜지는지는 Luna Overdrive(미설계, "10초간 무한마나/쿨타임제로")가 소유. 이 문서는 훅의 존재와 "켜지면 코스트/쿨다운 검사를 완전히 건너뛴다"는 동작 계약만 소유. **GAS 구현 계약**: 표준 `CommitAbility()`는 Cost+Cooldown GE를 원자적으로 함께 커밋하므로 "우회 중 쿨다운 GE가 진입 자체를 안 함"(Edge Cases 참조)을 표현할 수 없다 — 구현은 `ActivateAbility()` 내에서 `CostBypass.Active` 태그 존재 시 `CommitAbility()` 호출 자체를 건너뛰는 명시적 분기를 사용한다("코스트/쿨다운은 GE로만" 일반 원칙의 문서화된 의도적 예외). `CanActivateAbility()` 오버라이드도 동일 태그를 검사해 코스트/쿨다운 체크를 스킵한다. 두 지점(활성화 가능 판정 + 커밋)이 같은 태그 하나를 보므로 판정 불일치 불가.
...
- **Combo/Tension Gauge** (하류, 미설계): `OnSpellCast(Element)` / `OnSpellHit(Element, Target)` 이벤트를 구독해 콤보 누적에 활용할 것으로 예상 — "무엇이 콤보 가치가 있는가"는 해당 GDD 소유, 이 문서는 이벤트 발행까지만.
...
- **Luna Overdrive** (하류, 미설계): Core Rule 10의 코스트/쿨다운 우회 훅을 소비 — 지속시간(10초)과 발동 조건은 해당 GDD 소유.
...
```
NewMana_PerTick = clamp(CurrentMana + ManaRegenRate * DeltaTime, 0, MaxMana)
```
...
| 같은 프레임에 서로 다른 두 속성(예: 블랙홀+화염) 캐스트 입력이 동시에 들어옴 | 두 캐스트를 완전히 독립적으로 처리 — 각자의 마나/쿨다운 게이트만 검사, 합산 비용/공유 쿨다운 없음 | Core Rule 6(속성별 개별 쿨다운) 계약 — 시너지(Spell Weaving)를 노리는 동시입력이 여기서 인위적으로 막히면 안 됨 |
...
| Combo/Tension Gauge (미설계) | 하류 (이 시스템에 의존) | `OnSpellCast`/`OnSpellHit` 이벤트 구독 예정 |
| Spell Weaving / Synergy (미설계) | 하류 (이 시스템에 의존) | 속성 GameplayTag + 히트 데이터 소비, 시너지 판정 시 `bBypassDefense` 오버라이드 |
| Luna Overdrive (미설계) | 하류 (이 시스템에 의존) | `CostBypass.Active` 태그를 통한 코스트/쿨다운 우회 훅 소비 |
