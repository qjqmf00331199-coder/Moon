...
* **위치 래그(Lag)**: 순간적인 대쉬나 공중 발사 시 카메라가 캐릭터에 너무 딱딱하게 붙어 화면이 심하게 요동치는 것을 막기 위해 `bEnableCameraLag = true`를 활성화한다. 회전 래그는 조준 반응성을 위해 끈다.
* **오프스크린 방지 (CameraLagMaxDistance)**: 고속 대쉬나 순간적인 위치 이동 시 캐릭터가 화면 밖으로 이탈하지 않도록 `CameraLagMaxDistance = 60.0 uu`를 설정한다. 이 거리를 초과하면 스프링 암이 즉시 추적한다.
...
* **해결책**: 스프링 암의 `CameraLagMaxDistance`를 `60.0 uu`로 제한하여 순간 이동 후에도 캐릭터가 카메라 중심에서 멀어지지 않도록 한다. 이 한계에 도달하면 보간 연산 대신 하드 플로우(Hard Follow)로 강제 고정된다.
...
|---|---|---|---|---|
| `TargetArmLength` | 450.0 uu | 250.0 ~ 500.0 uu | 캐릭터가 작게 보이고 시야가 넓어져 전황 파악이 쉽지만 피격 및 세부 모션의 실감도 하락. | 캐릭터가 크게 보이고 현장감이 상승하지만 적들의 배치 파악이 어려워짐. |
| `CameraSocketOffset` | (X=0.0, Y=45.0, Z=20.0) | Y: -50 ~ 50, Z: -30 ~ 50 | 카메라의 중심축이 오른쪽/위쪽으로 밀려 비대칭 숄더뷰 연출 가능. | 카메라의 중심축이 왼쪽/아래쪽으로 밀림. |
...
| `CameraLagSpeed` | 18.0 | 5.0 ~ 20.0 | 카메라 추적이 매우 신속해지고 지연이 최소화됨. 과도할 경우 즉각반응성 스냅처럼 움직여 떨림 발생 가능. | 카메라 추적이 부드러워져 모션 멀미가 감소하나 빠른 대쉬 시 캐릭터가 화면 구석으로 밀림. |
| `CameraRotationLagSpeed` | 15.0 | 8.0 ~ 25.0 | 마우스/스틱 회전에 따른 시점 이동 스무딩이 신속해짐. | 마우스 조작 시 화면이 한 템포 미끄러지듯(Slippery) 늦게 회전하는 불쾌감 유발. |
| `CameraLagMaxDistance` | 60.0 uu | 40.0 ~ 120.0 uu | 고속 급 가속 시 캐릭터가 카메라 중심에서 더 멀리 벗어나 날아가는 속도감 증폭. | 캐릭터가 항상 카메라 중앙 근처에 엄격하게 강제 고정되어 딱딱해짐. |
...
* **THEN**: 카메라와 캐릭터 간의 월드 거리는 `CameraLagMaxDistance`인 `60.0 uu`를 절대로 초과할 수 없으며, 한계 도달 시 카메라는 지연 없이 캐릭터를 하드 트래킹해야 한다.
...
* **THEN**: 카메라의 스프링 암 길이(`TargetArmLength`)는 `450.0 uu` 상태를 정상 유지해야 하며, 파편으로 인해 카메라가 캐릭터 쪽으로 갑작스럽게 스냅되는 현상이 전혀 없어야 한다.
...
* **THEN**:
  ...
  3. 처형 종료 후 원래 3인칭 기본 시점(`TargetArmLength = 450.0 uu`, `SocketOffset = (0,45,20)`)으로 `0.3`초에 걸쳐 정상 복귀되며, 회전 조작 기능도 복구되어야 한다.
