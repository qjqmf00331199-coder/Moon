...
`production/overnight-queue.md`의 태스크를 시키고, 결과를 Gemini로 3줄 요약해서 다시 Discord로
...
- **`!야간시작`**
  `production/overnight-queue.md`에 정의된 태스크 4개(스파이크 리버스도큐먼트, registry 팩트체크
...
2. 각 파일을 `production/overnight-queue.md`에 적힌 "Review checklist" 기준으로 확인
...
`production/overnight-queue.md`에 새 태스크를 문서로 먼저 추가하고(사람이 읽는 스펙),
