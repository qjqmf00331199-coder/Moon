...
이 봇은 Discord 채팅으로 명령을 보내면, 레포 루트의 `OLLAMA-INSTRUCTIONS.md`를 직접 파싱해서
그 안의 `[ ]`(아직 안 돌린) 태스크를 로컬 Ollama(qwen2.5-coder:7b)에게 시키고, 결과를 Gemini로
3줄 요약해서 다시 Discord로 알려주는 스크립트다. 결과물은 전부 draft — `production/overnight-output/`
에 저장될 뿐, 이 봇은 `design/gdd/` 등 실제 프로젝트 파일을 절대 직접 건드리지 않는다.
...
'!야간시작' = OLLAMA-INSTRUCTIONS.md의 큐 태스크 순차 실행 / '!작업 <내용>' = 자유 작업 1건 실행
```
...
- **`!야간시작`**
  레포 루트 `OLLAMA-INSTRUCTIONS.md`를 읽어서 그 안의 `[ ]`(아직 안 돌린) 태스크를 전부 순서대로
  실행 — 몇 개인지는 그 파일 상태에 달림, 봇 코드에 개수가 고정돼있지 않음. 태스크마다 완료
  메시지 + Gemini 3줄 요약을 채팅에 보여주고, 전체 결과 파일은 `production/overnight-output/`에
  저장됨.
...
2. 각 파일을 `OLLAMA-INSTRUCTIONS.md`에 적힌 "Review checklist" 기준으로 확인
...
봇은 `TASKS` 같은 별도 리스트를 코드에 갖고 있지 않다 — 레포 루트의 `OLLAMA-INSTRUCTIONS.md`를
`!야간시작` 실행 시점에 직접 파싱한다. 새 태스크는 그 파일에 한 곳에만 추가하면 되고, 코드를
따로 고칠 필요는 없다.

단, 파싱이 정규식 기반이라 아래 "Format contract" 형식을 정확히 지켜야 함(`OLLAMA-INSTRUCTIONS.md`
맨 위에도 같은 내용이 적혀있음):
- 헤딩: `## [ ] Task N — 제목`
- `**Context files**:` 다음 줄부터 `- {{PLACEHOLDER}}: REPO_ROOT 기준 상대경로` 한 줄씩
- `**Prompt**:` 다음에 fenced 코드블록(\`\`\`) 하나 — 그 안에 Context files의 placeholder가
  전부 등장해야 하고, "Task 2와 동일" 같은 참조 없이 완전히 자기완결적이어야 함
- `**Output path**: \`production/overnight-output/....md\``

형식이 하나라도 어긋난 태스크 블록은 에러 없이 조용히 스킵된다 — 봇이 죽지는 않지만 그 태스크는
그날 밤 실행되지 않으므로, 추가 후 `python -c "..."`로 직접 파싱 결과를 확인해보는 게 안전하다.
