...
| **Knowledge Risk** | LOW — no post-cutoff API is required. Every mechanism this ADR uses (`FGameplayAttributeData` clamp in `PreAttributeChange`, `GetGameplayAttributeValueChangeDelegate`, per-actor `Tick`-based accumulation) is the same pre-cutoff-stable GAS/engine primitive set ADR-0001 and ADR-0004 already use safely. |
| **References Consulted** | `docs/engine-reference/unreal/VERSION.md` (no domain-specific GAS/Tick changes flagged in the 5.7→5.8 delta); existing project code (`MoonCharacterBase.cpp`/`.h`, `MoonAttributeSet.cpp`/`.h`) |
| **Post-Cutoff APIs Used** | None. |
| **Verification Required** | Two items, both found during this ADR's `ue-gas-specialist` validation pass: (1) confirm no Gain-triggering call site (ability activation, AnimNotify) can execute in a tick group later than `TG_PostUpdateWork` in the same frame — the precondition Decision 3's ordering guarantee depends on; (2) confirm `FOnAttributeChangeData::GEModData` is non-null only for GameplayEffect-originated attribute writes against real 5.8 GAS headers, before Decision 5's filter is implemented as written. |
...
3. **TR-tension-004 — same-frame Gain→Penalty→Decay ordering is enforced by resolving Penalty+Decay in a dedicated, late-tick-group function, not by leaving them in `AMoonCharacterBase`'s normal `Tick()`.** Because `AddTensionFromJustDodge()` (Dash) and `ApplyTensionDamagePenalty()` (Health/Damage Core) are independent call sites with no inherent relative-ordering guarantee within a frame, a naive "defer Penalty into the Character's regular `Tick()`" fix is not actually sufficient by itself: if a Gain-triggering call (e.g. a Just-Dodge ability activation) happens to fire *after* that `Tick()` call in the same frame — which the Character's own default tick group does not structurally rule out — a Penalty flagged earlier that frame would already have resolved before the late Gain, producing Penalty-then-Gain instead of the mandated Gain-then-Penalty (materially different results: (60×0.8)+20=68 vs. mandated (60+20)×0.8=64). This ADR closes that hole structurally rather than accepting the race:
   ...
   - **This still depends on one unverified precondition** (Engine Compatibility, Verification Required): that no foreseeable Gain-triggering call site executes even later than `TG_PostUpdateWork` in the same frame (e.g. a hypothetical latent `AbilityTask` completion callback, or a Blueprint event bound to a later tick phase). A `gameplay-programmer` engine-spike confirmation of this is required before this Decision is treated as fully closing TR-tension-004, not merely asserted here.
...
5. **Damage Penalty trigger reuses the existing Health attribute-change delegate, gated on GameplayEffect origin — no new `OnDamageApplied` interface.** `combo-tension-gauge.md` names `OnDamageApplied` as an interface Health/Damage Core has not yet exposed ("상류 문서가 '추후' 노출 예정으로 명시한 인터페이스" — the GDD itself flags this as provisional). Rather than adding a new delegate to ADR-0008's already-Accepted surface, this ADR satisfies the requirement with the interface that already exists: `UMoonAbilitySystemComponent`'s generic `GetGameplayAttributeValueChangeDelegate(GetHealthAttribute())` (the same one Combat HUD's `HandleHealthChanged` already binds). `AMoonCharacterBase` subscribes its own handler to this delegate; it calls `ApplyTensionDamagePenalty()` (arming the Decision 3 pending-flag) only when **both** `Data.NewValue < Data.OldValue` (a decrease) **and** `Data.GEModData != nullptr` (the change originated from a GameplayEffect execution).
   The second condition is required, not optional: ADR-0008 Decision 5 handles `TR-hp-008` (runtime `MaxHealth` re-clamp) by writing `Health = min(Health, NewMaxHealth)` directly inside `PreAttributeChange(MaxHealth)` — a genuine Health-attribute decrease with no damage GE involved, which would otherwise spuriously trip this ADR's damage-penalty filter on every `MaxHealth` shrink. `FOnAttributeChangeData::GEModData` is non-null only when the change came from a `GameplayEffect`'s execution (which the actual damage-application path always is) and null for a direct attribute-set path like `PreAttributeChange`'s `MaxHealth` reclamp — so gating on both conditions together excludes the reclamp while still catching every real damage hit. (This must be confirmed against real 5.8 GAS headers before implementation — see Engine Compatibility, Verification Required — the reasoning is standard pre-cutoff GAS behavior, but this ADR does not assert it as verified fact.)
...
- **`TG_PostUpdateWork`'s ordering guarantee depends on an unverified precondition** (Engine Compatibility, Verification Required item 1): that no Gain-triggering call site (ability activation, AnimNotify) ever executes later than `TG_PostUpdateWork` in the same frame. This is standard-case true for normal input-driven ability activation and AnimNotify firing, but a `gameplay-programmer` engine spike must confirm it holds for this project's actual ability/animation setup — including any future latent `AbilityTask` completion path — before this Decision is treated as fully closing TR-tension-004.
...
7. Have `gameplay-programmer` confirm, via an engine spike, that no Gain-triggering call site in this project's actual ability/animation setup executes later than `TG_PostUpdateWork` in the same frame (Risks) — required before Decision 3 is treated as fully verified, not blocking for Accept.
